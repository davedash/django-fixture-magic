...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            if ids[0] == '*':
                objs = dump_me.objects.using(using).all()
            else:
                objs = dump_me.objects.using(using).filter(pk__in=[int(i) for i in ids])
        except ValueError:
            # We might have primary keys that are longs...
            try:
                objs = dump_me.objects.using(using).filter(pk__in=[long(i) for i in ids]) 
            except ValueError:
                # Finally, we might have primary keys that are strings...
                objs = dump_me.objects.using(using).filter(pk__in=ids)
        ...
        if options.get('kitchensink'):
            ...
            for obj in objs:
                for rel in related_fields:
                    try:
                        add_to_serialize_list(obj.__getattribute__(rel).(using).all(), using)
                    except FieldError:
                        pass
                    except ObjectDoesNotExist:
                        pass

        add_to_serialize_list(objs, using)
        serialize_fully()
